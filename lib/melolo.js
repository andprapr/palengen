...
const meloloLinkStream = async (videoId) => {
  // Coba beberapa variasi parameter biz_param
  const paramVariations = [
    // Variasi 1: Original dengan video_id_type = 0
    {
      biz_param: {
        source: 4,
        use_os_player: false,
        video_id_type: 0,  // 0 untuk video ID biasa
        video_platform: 3,
    ...
    // Variasi 3: Dengan series context
    {
      video_id: String(videoId),
      biz_param: {
        detail_page_version: 0,
  ...
  for (let i = 0; i < paramVariations.length; i++) {
    const data = paramVariations[i];
    
    try {
      console.log("[Melolo Stream] ============================================");
      console.log(`[Melolo Stream] Trying variation ${i + 1}/${paramVariations.length}`);
      console.log("[Melolo Stream] video_id:", videoId);
      ...
      // Jika sukses (code = 0), return result
      if (response.data?.code === 0) {
        console.log("[Melolo Stream] ✅ SUCCESS with variation", i + 1);
      ...
      // Jika error, log dan coba variasi berikutnya
      console.warn(`[Melolo Stream] ❌ Variation ${i + 1} failed with code:`, response.data?.code);
      console.warn("[Melolo Stream] Error message:", response.data?.message || response.data?.BaseResp?.StatusMessage);
      
    } catch (error) {
      console.error(`[Melolo Stream] ❌ Variation ${i + 1} threw error:`, error.message);
      // Continue to next variation
    }
  }
  
  // Jika semua variasi gagal
  console.error("[Melolo Stream] ============================================");
  console.error("[Melolo Stream] ALL VARIATIONS FAILED!");
  console.error("[Melolo Stream] Tried", paramVariations.length, "different parameter combinations");
  
  return {
    status: false,
    code: 101000,
    message: "All parameter variations failed. Video stream not accessible.",
    data: {},
    error: "API Error: Unable to get stream URL with any known parameter combination",
    _debug: {
      variations_tried: paramVariations.length,
      video_id: videoId,
      suggestion: "This videoId might not be valid or the video might require VIP access"
    }
  };
};
