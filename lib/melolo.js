...
const meloloLatest = async () => {
  try {
    ...
    console.log('=== MELOLO LATEST RAW RESPONSE ===');
    console.log('Full response:', JSON.stringify(response.data, null, 2));
    console.log('Data array length:', response.data.data?.length);
    console.log('Index [2]:', JSON.stringify(response.data.data?.[2], null, 2));
    
    const rawData = response.data.data[2];
    ...
    const normalizedBooks = books.map(book => ({
      ...book,
      book_id: book.search_source_book_id || book.book_id,
      id: book.search_source_book_id || book.book_id,
      book_name: book.book_name || book.title,
      thumb_url: book.thumb_url || book.cover
    }));
    
    console.log('Normalized books count:', normalizedBooks.length);
    if (normalizedBooks.length > 0) {
      console.log('First normalized book:', JSON.stringify(normalizedBooks[0], null, 2));
    }
