...
const meloloDetail = async (seriesId) => {
  try {
    ...
    console.log("[Melolo Detail] Response structure:", JSON.stringify(response.data).substring(0, 300));
    
    // Parse response sesuai struktur yang benar dari JSON
    const rawData = response.data?.data || {};
    const videoData = rawData.video_data || {}; // Data ada di video_data, bukan series_info!
    const videoList = videoData.video_list || [];
    
    // Log info
    console.log("[Melolo Detail] Series name:", videoData.series_title || "Unknown");
    ...
    console.error("[Melolo Detail] Request URL:", error.config?.url || "Unknown");
